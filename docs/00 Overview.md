...
- **Cross-cutting profiles apply to every engineer**, regardless of surface: [AI Tooling](role-profiles/ai-tooling.md) and [Production & Operations](role-profiles/production-operations.md).
- **The one filled domain profile is [Web Frontend](role-profiles/web-frontend.md).** Roomle's domains are not clean partitions — Web Frontend, [3D / Configurator (Web SDK)](role-profiles/3d-configurator.md), and the embedding library overlap heavily, and most web engineers work across all three. Until the adjacent profiles are written, **engineers working on the 3D/SDK or embedding surfaces should read Web Frontend as their closest available proxy**, supplemented by their own judgment about what their surface adds.
- **Other domains are placeholders only**: [Backend (RAPI)](role-profiles/backend.md), [Infrastructure & Delivery](role-profiles/infrastructure.md), [3D / Configurator (Web SDK)](role-profiles/3d-configurator.md), [Core (C/C++)](role-profiles/core-cpp.md). The placeholders exist so the ladder is structurally complete; treat them as known gaps, not endorsed content.
...
**Most external hires start at Expert**, although they can advance fast. After 1 month and then after 3 months a **ladder placement review** will take place to find the exact position in the Roomle Career Ladder. This is an internal ladder placement review, not a Probezeit assessment — the Austrian Probezeit under § 19 Abs 2 AngG ends after at most 1 month, and the 3-month review has no bearing on the continuation of the employment relationship. We will explain later why these reviews exist. The initial placement in the Roomle Career Ladder has nothing to do with salary or "Einstufung by IT KV". The Junior level is reserved for career starters (graduates, career changers, or trainees) who need structured guidance and onboarding.
...
**IT-KV classification follows actual work, not the internal title.** Classification under the IT-KV (Verwendungsgruppe) is determined solely by the work an engineer actually performs, in line with the collective agreement. The internal ladder title is never a means to justify a lower IT-KV classification. Where the internal title lags behind the actual scope of work, the IT-KV classification — not the internal title — is decisive.

**The Senior L1 → Senior L2 transition is a deliberate choice.** Moving from L1 to L2 is not automatic — it requires mutual agreement. L2 brings a different kind of work: multiplier responsibilities (team standards, architecture across the team, growing others) on top of strong individual delivery. Anyone choosing this path should be aware that L2 is a **shared commitment** between the company and the engineer: the company commits to actively *supporting* the engineer's growth toward Lead-level responsibilities (mentoring, scope, learning budget, time, honest feedback), and the engineer commits to actively *engaging* with that growth (taking on the multiplier work, accepting feedback, investing in the craft of leadership). Both sides should align on this expectation openly before promotion. The commitment is a shared expectation about how the role will be lived, not a contractual duty owed in either direction.
...
| **Homag Intelligence (HI)**                       | The Homag-content variant of Rubens Room Designer (calc.js + glue layer); engineering work spans Rubens, Web SDK, embedding, and a direct Roomle-dev ↔ Homag-dev channel                  | Cross-cutting; anyone working HI surfaces must hold the direct technical conversation with Homag developers |
...
HR-Leads are **not a substitute for a works council (Betriebsrat)**. The establishment of a Betriebsrat under § 40 ArbVG is in no way restricted, discouraged, or replaced by the HR-Lead system. The two address different needs and can co-exist.
...
2. **Compensating strength is acknowledged, but does not waive a missing axis.** Exceptional strength on one axis does not make up for a missing one: an engineer who is outstanding on Technical Mastery but does not yet meet the Influence bar has not yet met the full level definition. That exceptional depth can still justify a higher salary at the current level (see [Compensation growth is possible at Senior L1…](#starting-level-progression)). The career step and the compensation step are two different conversations — the backdoor is in compensation, not in the level definition.
...
This document describes an orientation framework for career development at Roomle. It does **not** create individual legal entitlements to promotion, a specific title, or a specific salary. All applicable provisions of the IT-KV, the Gleichbehandlungsgesetz (GlBG), the Arbeitsverfassungsgesetz (ArbVG), the Angestelltengesetz (AngG), and the GDPR take precedence over anything written here. Assessment results and documented growth plans are processed in accordance with the GDPR; data-subject rights (access, rectification, erasure) remain unaffected. Roomle reserves the right to amend this framework.
