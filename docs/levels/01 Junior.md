...
> **Current organisational note.** Roomle does not currently hire Juniors (but this can change anytime soon, also all current employees already grew out of Junior Role) — new hires typically start at Expert or higher. At the moment this level is documented for completeness and to define expectations if/when a Junior is hired (e.g., a graduate, career changer, or trainee). See [Junior → Expert](../background/boundaries/01-junior-vs-expert.md) for context.
...
* **Applies patterns**: uses established patterns, libraries, and tools to implement well-defined tasks. Follows existing conventions in the codebase rather than inventing new approaches.

* **Guided implementation**: can implement features and fixes when given clear requirements and pointers to relevant code. Produces working code that meets the specification with some review iterations.

* **Reads and understands code**: can read existing code and understand its intent. Uses the codebase as a learning resource to understand how things are done.

* **Basic debugging**: can debug straightforward issues using standard tools (logs, breakpoints, error messages). Knows how to read a stack trace and narrow down where a problem occurs.

* **Follows standards**: adheres to the team's coding standards, formatting rules, and PR conventions. Applies feedback from code reviews to future work.
...
**tldr;** A Junior participates in team efforts, communicates the status of their own work, asks for help when needed, and contributes to a respectful team environment.

* **Participates**: shows up prepared to meetings, standups, and discussions. Shares what they're working on and what's blocking them.
* **Asks for help**: knows when to reach out. Doesn't struggle alone for too long, but also shows they've tried before asking.
* **Communicates status**: keeps tickets updated, writes clear PR descriptions, and responds to review comments. Others don't have to chase them for information.
* **Respectful interaction**: treats all colleagues with respect. Listens before speaking. Doesn't interrupt or dismiss others' input.
* **Open to pairing**: willing to pair-program, shadow senior engineers, or work together on tasks. Uses collaborative work as a learning opportunity.
* **Written communication**: writes clearly in tickets, PRs, and messages. Is learning to communicate technical information in an understandable way.
